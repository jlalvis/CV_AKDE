...
def adap_ks_out(coords, dim, K, obs, nsims):
    n = len(coords)-1
    refer = coords[obs]
    coords = np.concatenate((coords[:obs],coords[obs+1:]), axis=0)
    ...
    for i in range(n):
        ...
        density = density + multivariate_normal.pdf(refer, labeled[i,:-1], bw**2)
